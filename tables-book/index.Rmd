...
for this book. For now you can generate a script to update to the latest 
versions of the packages yourself by running this function:

```{r, eval = FALSE}
getInstallScript <- function() {
  ...
  if (!nrow(deps)) return("# All up to date, no installs needed")
  ...
  sprintf('%s("%s") \t%s', cmd, deps$ref, reason)
}

cat(getInstallScript(), sep = "\n")
```
