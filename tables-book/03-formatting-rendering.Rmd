...
If the format you need is not supported by the package you
would like to use, there are still options.  For example, tables produced
in `rtables` and `tables` can be converted to `flextable` objects,
giving access to the other output formats.
Pandoc [@pandoc]
