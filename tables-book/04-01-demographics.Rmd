...
This package creates similar tables to other packages but requires the dataset to be in a specific format. 
In this bookdown `data_demog` and `data_ae` are used within `tfrmt` examples (these datasets are stored in this package too). 
These are based on the CDISC pilot data but have been modified into ARD - hence, differ from the other packages examples. 

To see how these datasets have been created please refer to the following: 
<ul>
<li> https://github.com/GSK-Biostatistics/tfrmt/blob/main/data-raw/create_data_demog.R </li>
<li> https://github.com/GSK-Biostatistics/tfrmt/blob/main/data-raw/create_data_ae.R </li>
</ul>
...
tfrmt(
  ...
  body_plan = body_plan(
    ...
    frmt_structure(group_val = "Age (y)",
                   label_val = c("Mean", "SD", "<65 yrs", "65-80 yrs", ">80 yrs"),
                   frmt("xx.x")),

    frmt_structure(group_val = "Sex",
                   label_val = c("Male", "Female"),
                   frmt("xx.x")),

    frmt_structure(group_val = "Race (Origin)",
                   label_val = c("Caucasian", "African Descent", "Hispanic", "Other"),
                   frmt("xx.x")) ),

  # Define columns
  col_plan = col_plan(-starts_with("ord"), -grp)

  ) %>%
  print_to_gt(data_demog %>%
                filter(rowlbl1 %in% c("Age (y)", "Sex", "Race (Origin)"),
                      !(column %in% c("Total", "p-value"))) )
```
