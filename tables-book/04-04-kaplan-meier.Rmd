...
First we define named vectors that will be used later as 
...
Now use `tabulator()` and `as_flextable()` together to create the table. 
...
heading <- tabular(Heading("")*count ~ Heading()*ARM, data = adsl)

part1 <- tabular( Heading("Subjects with serious adverse events")*
           events*Heading()*valuepercent*Arguments(ARM = TRT01A) ~ 
           Heading()*TRT01A, 
         data = surv_tbl )
