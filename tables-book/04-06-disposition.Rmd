...
disp_subset <- merge(cadsl, adsl_tot, by = "TRT01A") |> 
  dplyr::filter(EOSSTT %in% c("COMPLETED", "DISCONTINUED")) |> 
  dplyr::mutate(
    DISCGRP = factor(
      ifelse(DCSREAS %in% c("ADVERSE EVENT", "DEATH"), "Safety", "Non-Safety"),
      levels = c("Safety", "Non-Safety")
    ),
    EOSSTT = factor(
      EOSSTT,
      levels = c("COMPLETED", "DISCONTINUED"),
      labels = c("Completed Study", "Discontinued Study")
    )
  )

disc_status <- disp_subset |> 
  dplyr::summarize(
    n = dplyr::n(),
...
disc_reason <- disp_subset |> 
  dplyr::filter(EOSSTT == "Discontinued Study") |> 
  dplyr::summarize(
    n = dplyr::n(),
    pct = dplyr::n()/mean(NTOT),
    .by = c(TRT01A, EOSSTT, DISCGRP, DCSREAS)
  ) |> 
  tidyr::pivot_wider(id_cols = c(EOSSTT, DISCGRP, DCSREAS), names_from = TRT01A, values_from = c(n, pct)) |> 
  dplyr::arrange(EOSSTT, DISCGRP, DCSREAS)

gt_disp <- dplyr::bind_rows(disc_status, disc_reason) |> 
  dplyr::mutate(
    EOSSTT = dplyr::if_else(is.na(DISCGRP), EOSSTT, "")
  ) 
...
gt_disp |> 
  ...
  tab_row_group(
    label = "Non-Safety",
    rows = DISCGRP == "Non-Safety"
  ) |> 
  row_group_order(
    groups = c(NA, "Safety", "Non-Safety") 
  ) |> 
  cols_hide(DISCGRP) |> 
  fmt_integer(
