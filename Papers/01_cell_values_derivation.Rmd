...
- `flextable`:
- `gt`: has a high-level and declarative interface for composing tables. Contains a variety of 'formatter' functions for transforming cell values from an input data frame to the desired reporting values (combines mutation and aesthetic styling with options for: scientific notation, uncertainty, ranges, percentages, suffixes, localized currency, etc.). Multiple output formats are supported with the same declarative interface (e.g., HTML, LaTeX/PDF, and RTF).
- `huxtable`:
- `mmtable2`:
